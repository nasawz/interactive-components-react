...
var WeixinShare = React.createClass({
    getDefaultProps: function(){
      return {
        ...
        wximgUrl:'',
        showOption:true,
        puzzleKey:'559e41a872dd5290337a136d'
      }
    },
    changeWx:function () {
    ...
    componentWillMount: function(){
      var url = 'http://p.baleina.cn/puzzle/api/'+this.props.puzzleKey+'/get_jsConfig';
      request.post(url).send({ url: window.location.href.split('#')[0] }).end(function(err, res) {
        var json = res.body;
        wx.config({
          debug: false,
          appId: json["appId"],
